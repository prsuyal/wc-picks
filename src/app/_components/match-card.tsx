...
export function MatchCard({ match, currentUserId }: { match: MatchWithPrediction; currentUserId: string }) {
  ...
  const [penaltyWinnerPred, setPenaltyWinnerPred] = useState<"home" | "away" | null>(
    (existing?.penaltyWinnerPred as "home" | "away" | null) ?? null,
  );
  ...
  const showPenPicker = isKnockout && !isLocked && isPredictedDraw;
  ...
  useEffect(() => {
    ...
    const existingPen = (existing?.penaltyWinnerPred as "home" | "away" | null) ?? null;
    ...
    const timeout = setTimeout(() => {
      ...
      upsert.mutate({
        ...
        homeScorePred: h,
        awayScorePred: a,
        penaltyWinnerPred: effectivePen,
      });
    }, 350);
  ...
  // Pen winner for locked draw predictions
  const lockedPenWinner =
    isKnockout && existing && existing.homeScorePred === existing.awayScorePred
      ? (existing.penaltyWinnerPred as "home" | "away" | null)
      : null;
        ...
        <div className="grid shrink-0 items-center justify-self-center sm:justify-self-end">
          {isLocked ? (
              ...
              {existing ? (
                <span className="text-muted-foreground text-center text-sm tabular-nums">
                  ...
                  <span className="mx-1 text-base leading-none">
                    {lockedPenWinner ? (lockedPenWinner === "home" ? homeFlag : awayFlag) : "–"}
                  </span>
                  {existing.awayScorePred}
                </span>
              ) : (
                <span className="text-muted-foreground text-center text-xs">no pick</span>
              )}
              {isLive ? (
            ...
            <div className="relative flex items-center">
              ...
              <div className="grid grid-cols-[3rem_auto_3rem] items-center gap-2">
                <Input
                  type="text"
                  inputMode="numeric"
                  pattern="[0-9]*"
                ...
                {showPenPicker ? (
                  <div className={`flex items-center gap-1${!penaltyWinnerPred ? " animate-pulse" : ""}`}>
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setPenaltyWinnerPred((p) => (p === "home" ? null : "home"));
                      }}
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setPenaltyWinnerPred((p) => (p === "away" ? null : "away"));
                      }}
